 ...
 */
	...
	public handleOver(io: InteractionObject, pointer: IPointer, ev: MouseEvent | TouchEvent, soft: boolean = false): void {
		...
		if (!io.isHover) {
			...
			// Triggered by some other event than genuine "over"?
			if (soft) {
				// Do nothing for now. Maybe something will come up later.
	 ...
	 * @param {boolean}                  soft     Invoked by helper function
	 */
	public handleOut(io: InteractionObject, pointer: IPointer, ev: MouseEvent | TouchEvent, soft: boolean = false): void {
	...
	public handleGlobalMove(pointer: IPointer, ev: MouseEvent | TouchEvent): void {
		...
		// interacted with
		// @todo remove this (event is prevent on pointerdown/touchstart, no need)
